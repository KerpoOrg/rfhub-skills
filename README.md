...
```bash
apm install -g KerpoOrg/rfhub-skills#v0.9.1
...
if your APM accepts `owner/repo#tag`: `KerpoOrg/rfhub-skills#v0.9.1`.
...
apm install --target claude,cursor \
  ...
  --transport http \
  --url "$RFHUB_MCP_URL" \
  --header "X-Rfhub-Skills-Version=0.9.1"
```
