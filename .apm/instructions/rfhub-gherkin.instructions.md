...
- Several business values needed later: do **not** put multiple `'${a}'` `'${b}'` on one first-level keyword. Each choose/set step stores one value with `Set Suite Variable    ${name}    ${value}`. Later steps in the **same case** read those variables, and the same value carries to another test case in the **same leaf** (`customer memorizes the order number` → `customer uses the memorized order number`). Suite variables do not cross hub leaves (each leaf is a separate Robot process). Do not hand off with `VAR … scope=TEST` / `scope=SUITE` — Robocop (`VAR06` `no-test-variable`, `MISC04`) flags test-scoped variables and `VAR` assignment signs; `Set Suite Variable` is Robocop-clean.
