...
4. Choose selection (do **not** preserve array order):
   ...
   - Scheduling: `parallelism` (`serial` | `suite` | `testcase`, default `suite`) works on any selection and overrides the wave’s mode.
   - Environment: `environment` (slug) — selects the orchestrator serving that environment and stamps the run. **Required** when several environments run the same `project`+`branch`.
