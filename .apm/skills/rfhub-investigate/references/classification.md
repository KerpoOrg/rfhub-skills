...
| **Faulty harness** | Process-shaped, not test-result-shaped: executor crash/timeout before test start, live "no-listener" hint, attachments never arrive, `source: "unavailable"` on metrics, same infra error across unrelated leaves/queues, parts never uploaded, acceptance gate cannot reach orch (`URLError` / network unreachable). Report via **rfhub-feedback**, not suite edits. Do **not** treat a green focused run as a substitute for a blocked gate. |
