...
1. Hub must already be up. Production hub is `https://rfhub.kerpo.org`. Hub
   ...
   unknown.
2. Preferred: install the MCP entry with APM's CLI, giving it only the URL. The
   client discovers OAuth from the unauthenticated `401` +
...
```
   ...
   Hub developers may name the local server `rfhub-dev` and point it at
   `http://127.0.0.1:2998/api/mcp`.

3. First connect opens the hub authorize URL in a browser — log in with Pocket
...
4. Manual fallback (clients without MCP OAuth, or scripted setups): mint a key
...
5. Fallback only when APM is not in use: merge into workspace `.cursor/mcp.json`
...
6. Reload MCP in Cursor (**Settings → MCP**). Confirm tools include
   `rfhub_queue`, `rfhub_failures`, `rfhub_metrics_overview`.
7. **Skills version header (required for mismatch warnings):** send
   ...
   train: `0.3.0`. MCP `initialize` / `tools/list` return
...
8. Optional stdio fallback is hub `mcp/server.mjs` with `HUB_ACCESS_KEY` —
   prefer the HTTP URL.
9. After connect, stop. Queue → `rfhub-queue`. Failures → `rfhub-investigate`.
...
- A later plain `apm install` removes undeclared MCP entries. If the remote MCP is installed with the CLI command above rather than kept in `apm.yml`, rerun the MCP install command after a normal package install.
