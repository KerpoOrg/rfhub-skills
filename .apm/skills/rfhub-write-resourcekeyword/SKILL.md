...
4. Leaves add `Resource    path.resource`. Do not put identity ids under `Test Tags` on the resource file unless every importer should inherit them (usually **do not** — `suite_id` belongs on the leaf). Shared product tags belong on suite `Test Tags` (**rfhub-write-suite**), not on the resource.
...
6. Keywords called from a test case take **one** inlined `'${arg}'` (single quotes) with a meaningful word after the variable. Several values: each Gherkin-facing keyword stores one value with `Set Suite Variable    ${name}    ${value}` — read by later steps in the same case, or by the next test case in this leaf (`customer memorizes the order number` / `customer uses the memorized order number`). First-level names stay human; deeper keywords get technical and eventually Python. Do not put two embedded args on one Gherkin-facing name. Deeper keywords may take arguments any way.
...
- `VAR` without `scope=` is keyword-local; later steps will not see the context. Hand off with `Set Suite Variable`; suite variables do not cross hub leaves. Do not use `VAR … scope=TEST` / `scope=SUITE` (Robocop `VAR06` / `MISC04`).
